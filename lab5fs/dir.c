...
static int lab5fs_readdir(struct file * f, void * dirent, filldir_t filldir) {
    ...
    printk("dir->ino = %lu, D_SIZE(blocknum) = %lu\n", dir->i_ino, D_SIZE(dir->i_blocks));
    ...
    if (f->f_pos >= D_SIZE(dir->i_blocks)) {
        unlock_kernel();
        return 0;
    }
    ...
    while ((set_bit_pos = find_next_bit((unsigned long *)dentry_map_p, dentry_map_size, set_bit_pos+1)) < dentry_map_size) {
        ...
        if (dentry_p->ino != 0) {
            int type = (dentry_p->ino == LAB5FS_ROOT_INO)? DT_DIR : DT_REG;
            printk("ino = %u, type = %d\n", dentry_p->ino, type);
...
static int lab5fs_unlink(struct inode * dir, struct dentry * dentry) {
    ...
    mark_inode_dirty(inode);
    printk("after unlink, D_SIZE(i_blocks) = %lu, dir->i_blocks = %lu\n",
            D_SIZE(dir->i_blocks), dir->i_blocks);
    error = 0;
...
static int lab5fs_link(struct dentry * old, struct inode * dir, struct dentry * new) {
    ...
    printk("lab5fs_link(), new->d_name.name = %s, ino = %lu\n", 
            new->d_name.name, inode->i_ino);
    
    lock_kernel();
    err = lab5fs_add_entry(dir, new->d_name.name, new->d_name.len, inode->i_ino);
    if (err) {
        unlock_kernel();
        return err;
    }
...
static int lab5fs_add_entry(struct inode * dir, const char * name, int namelen, int ino) {
    ...
    if (!dentry_p->ino) {
        //dir->i_size += LAB5FS_DENTRYSIZE;
        dir->i_blocks ++;
        printk("LAB5FS_I(dir)->i_blocksnum = %lu, D_SIZE(i_block) = %lu\n", 
            dir->i_blocks, D_SIZE(dir->i_blocks));
...
static struct buffer_head * lab5fs_find_entry(struct inode * dir, const char * name, 
    ...
    printk("dir->ino = %lu, D_SIZE(i_blocks) = %lu\n", dir->i_ino, D_SIZE(dir->i_blocks));
